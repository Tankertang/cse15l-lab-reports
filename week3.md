...
For the `string = string + store[1]+"\n";` method, it takes the `<string>` value after the " = "  in the url(`/add-message?s=<string>
`)and concatenate it with whatever was previously stored in the `string` variable (Note: the `string` variable is initially declared empty, ex. `string = ""`) then indent a new line. Then that string is stored in the variable `string` to be used again whenever the user make another url.
