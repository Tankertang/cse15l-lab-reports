...
For the `string = string+store[1]+"\n";` method, it takes the the <string> value after the "=" in url and concatenate it with whatever was previously strong in the       `string` variable then indent a new line. Then that string is stored in the variable `string` to be used again whenever the user make another url.
